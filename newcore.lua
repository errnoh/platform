...
-- Initialized in CoreInitialize() --

-- Is the player on Legion (lanes go towards top of the map)
...
function core.CoreInitialize(controller)
	...
	if core.myTeam == HoN.GetLegionTeam() then
        ...
        core.InitializeItems()
	
	--get useful buildings
	local units = HoN.GetUnitsInRadius(Vector3.Create(), 99999, core.UNIT_MASK_ALIVE + core.UNIT_MASK_BUILDING)
	
	if bDebugEchos then BotEcho("building units. count:"..core.NumberElements(units)) end
	if bDebugEchos then BotEcho("Gathering and sorting buildings") end
...
function core.AssessLocalUnits(botBrain, vecPosition, nRadius)
	...
	StartProfile('GetUnits')
		local tUnits = botBrain:GetLocalUnits() --XXX:Not documented
		local tSortedUnits = botBrain:GetLocalUnitsSorted() --XXX:Not documented
	StopProfile()
...
--WTF, exported godfunction that no-one would actually use. Shouldnt be in API
...
function core.SortUnitsAndBuildings(tUnitList, tSortedUnits, bHeroesAsMemoryUnits)
	...
	tSortedUnits = tSortedUnits or {}
        tSortedUnits = core.SortBuildings(tUnitList, tSortedUnits)
	...
	for key,curUnit in pairs(tUnitList) do
		if curUnit ~= core.unitSelf then  
			...
			if curUnit:GetTeam() == allyTeam then
				...
				if not curUnit:IsBuilding() then
...
function core.SortBuildings(tBuildingList, tSortedTable)
	...
	for key, building in pairs(tBuildingList) do
		if bDebugEchos then BotEcho("SortBuildings - building: "..tostring(building:GetTypeName())) end
